...
| P5 | 开奖设置 | [NEW] | 运营后台 | 配置开奖日和开奖时间 |
| P6 | 前端活动页 | [NEW] | 用户端 | 展示七夕主题页、浪漫值、宝箱状态和领取入口 |
...
- 参数区域顺序：基础设置、皮肤选择、浪漫值签到体系、充值加速、开奖设置、其他设置。
...
| CC-5 | 开奖日早于活动开始时间 | 保存时拦截，并提示开奖日需在活动周期内 |
| CC-6 | 用户充值方式不符合配置 | 不增加浪漫值，保留原活动状态 |
| CC-7 | 用户当日重复签到 | 不重复增加浪漫值，展示今日已签到 |
| CC-8 | 用户未到开奖时间点击开启 | 展示敬请期待或倒计时，不播放开奖动画 |
| CC-9 | 用户已领取后再次进入 | 展示已领取状态和领取时间 |
| CC-10 | 每小时发送次数为空、非整数或越界 | 保存时拦截，并提示填写 0 至 999999 的整数 |
| CC-11 | 前端展示关闭 | 用户端不展示活动入口 |
| CC-12 | 皮肤选择为空 | 保存时拦截，并提示选择活动皮肤 |
...
| 5 | 开奖设置 | 支持配置开奖日和开奖时间，并保存回显 | 编辑开奖设置并保存回显 | - [ ] |
| 6 | 前端隐藏奖励 | 开奖前用户端仅展示浪漫值，不展示真实奖励金额 | 使用未开奖账号查看活动页 | - [ ] |
| 7 | 开奖效果 | 达到开奖条件后播放系统默认开奖效果 | 开启宝箱并观察开奖过程 | - [ ] |
| 8 | 奖励领取 | 用户开奖后可领取奖励，领取后展示已领取 | 完成领取并核对领取记录 | - [ ] |
| 9 | 通知发送参数 | 其他设置中支持站内信模板、开始发送时间、每小时发送次数 | 填写边界值并保存 | - [ ] |
| 10 | 前端设计一致性 | 选择七夕神秘大奖皮肤后，前端状态和按钮与设计稿保持一致 | 对照设计稿验收 | - [ ] |
...
| 3 | 可配置性 | 运营可独立调整基础信息、皮肤、浪漫值曲线、充值加速和开奖设置 | 后台保存并回显 | - [ ] |
