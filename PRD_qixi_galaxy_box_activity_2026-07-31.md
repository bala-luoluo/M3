...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---|---|
| 完成借呗加速值配置 | 数字输入 | 是 | 15% | 用户完成借呗后额外增加的浪漫值百分比 |

#### 4.5.2 规则说明
- 用户在活动期间完成借呗后，可获得一次浪漫值加速机会。
- 加速值按百分比配置，前端展示为“+配置值% 浪漫值”。
- 借呗加速与充值加速分别计算，用户最终浪漫值按活动规则累计展示。
...
| CC-5 | 借呗加速值为空、非数字或小于 0 | 保存时拦截，并提示填写有效百分比 |
...
| 5 | 借呗加速 | 支持配置完成借呗加速值百分比 | 保存配置后使用完成借呗用户验证浪漫值增加 | - [ ] |
