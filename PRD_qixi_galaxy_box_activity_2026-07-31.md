...
| P4 | 开奖与奖励配置 | [NEW] | 运营后台 | 配置开奖日和开奖时间 |
...
| CC-5 | 用户当日重复签到 | 不重复增加浪漫值，展示今日已签到 |
| CC-6 | 用户未到开奖时间点击开启 | 展示敬请期待或倒计时，不播放开奖动画 |
| CC-7 | 用户已领取后再次进入 | 展示已领取状态和领取时间 |
| CC-8 | 每小时发送次数为空、非整数或越界 | 保存时拦截，并提示填写 0 至 999999 的整数 |
| CC-9 | 前端展示关闭 | 用户端不展示活动入口 |
| CC-10 | 皮肤选择为空 | 保存时拦截，并提示选择活动皮肤 |
...
| 6 | 奖励领取 | 用户开奖后可领取奖励，领取后展示已领取 | 完成领取并核对领取记录 | - [ ] |
| 7 | 通知发送参数 | 其他设置中支持站内信模板、开始发送时间、每小时发送次数 | 填写边界值并保存 | - [ ] |
| 8 | 前端设计一致性 | 选择七夕神秘大奖皮肤后，前端状态和按钮与设计稿保持一致 | 对照设计稿验收 | - [ ] |
...
| 3 | 可配置性 | 运营可独立调整基础信息、皮肤、浪漫值曲线和开奖时间 | 后台保存并回显 | - [ ] |
